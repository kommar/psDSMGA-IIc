...
void DSMGA2::oneRun (int iteration, bool output, Chromosome* bestIndividual) {

    if (CACHE)
        Chromosome::cache.clear();

    mixing(bestIndividual);

    double max = -INF;
    stFitness.reset ();

    for (int i = 0; i < nCurrent; ++i) {
		updateBestStatistics(population[i], max);
    }

	if (bestIndividual) {
		updateBestStatistics(*bestIndividual, max, false);
	}
...
void DSMGA2::restrictedMixing(Chromosome& ch, Chromosome* bestIndividual, bool performBackMixing) {
	if (!twoEdge) {
		...
		if (taken && performBackMixing) {
			...
			if (bestIndividual)
				backMixing(ch, mask, *bestIndividual);
		}
	} else {
...
bool DSMGA2::backMixing(Chromosome& source, list<int>& mask, Chromosome& des) {
    ...
    for (list<int>::iterator it = mask.begin(); it != mask.end(); ++it) {
		if (trial.getVal(*it) != source.getVal(*it))
			anyDifferent = true;
...
void DSMGA2::comparativeMixing(int chIndex, Chromosome* bestIndividual) {
	int r = myRand.uniformInt(0, ell - 1);
	...
	if (taken) {
		...
		for (int i = 0; i < nCurrent; ++i) {
			if (shouldBackMixing(population[chIndex], mask)) {
				if (EQ)
...
void DSMGA2::mixing(Chromosome* bestIndividual) {
    ...
    for (int k=0; k<repeat; ++k) {
        ...
        for (int i=0; i<nCurrent; ++i) {
			if (useComparativeMixing) {
				comparativeMixing(orderN[i], bestIndividual);
			} else {
				restrictedMixing(population[orderN[i]], bestIndividual, true);

				if (bestIndividual) {
					restrictedMixing(*bestIndividual, nullptr, false);
				}
			}
...
void DSMGA2::updateBestStatistics(Chromosome& ch, double& max, bool recordFitness) {
	double unitation;
	double fitness = ch.getFitness();
	
	if (fitness > max) {
		max = fitness;
		unitation = ch.getUnitation();
	}
