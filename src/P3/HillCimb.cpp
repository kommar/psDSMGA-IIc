...
void hill_climb::first_improvement(Random & rand, vector<bool> & solution,
                                   float & fitness, Evaluator& evaluator,
									bool  bMultiObjDominationFitnessCompare) {
  ...
  do {
    ...
    for (const auto& index : options) {
	  ...
	  if (bMultiObjDominationFitnessCompare == false)
	  {
		  //original behavior
	  ...
	  {
		  if (b_first_time == true)
		  {
			  evaluator.vSetReferenceSolutionForMO(solution);
			  b_first_time = false;
		  }//if (b_first_time == true)
		   ...
		   // flip and evaluate the modification
			...
			fitness = evaluator.evaluateMO(solution);
			if (fitness > 0) {
				// Keep change, update variables
				fitness = new_fitness;
				improvement = true;
		  ...
		  //iCompareMO_Solutions
	  }//else  if (bMultiObjDominationFitnessCompare == false)*/
...
void hill_climb::steepest_ascent(Random & rand, vector<bool> & solution,
  ...
  do {
    ...
    if (improved) {
      ...
      solution[previous] = !solution[previous];
      bests.clear();
    }
  } while (improved);
}
