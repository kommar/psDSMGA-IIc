...
bool Population::donate(vector<bool> & solution, float & fitness,
                        ...
                        Evaluator& evaluator, bool  b_multi_obj_domination_fitness_compare) {

	vector<bool> v_prev_solution;
	if (b_multi_obj_domination_fitness_compare == true)  v_prev_solution = solution;
  ...
  if (b_multi_obj_domination_fitness_compare == false)
  {
	  //original
	  if (changed) {
  ...
  {
	  if (changed) {
		  ((Injected&)evaluator).vSetReferenceSolutionForMO(v_prev_solution);
		  float new_fitness = ((Injected&)evaluator).evaluateMO(solution);
  ...
  }//else  if (b_multi_obj_domination_fitness_compare == false)
...
void Population::improve(Random& rand, vector<bool> & solution, float & fitness,
                         Evaluator& evaluator,
						bool  b_multi_obj_domination_fitness_compare) {
