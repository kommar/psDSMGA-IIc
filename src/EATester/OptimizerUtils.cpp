...
template <class TGenotype, class TFenotype>
CSteadyStateOptimizer<TGenotype, TFenotype> * OptimizerUtils::pcGetSteadyStateOptimizer(CProblem<TGenotype, TFenotype> *pcProblem, CLog *pcLog, uint32_t iRandomSeed, istream *psSettings, CError *pcError, bool bIsObligatory)
{
	...
	{
		if (!dynamic_cast<CSteadyStateOptimizer<TGenotype, TFenotype>*>(pc_optimizer))
		{
			pcError->vSetError(CError::iERROR_CODE_OPERATOR_NOT_FOUND, "optimizer instead of steady state optimizer");
		}//if (!dynamic_cast<CSteadyStateOptimizer<TGenotype, TFenotype>*>(pc_optimizer))
	}//if (!*pcError && pc_optimizer)

	return (CSteadyStateOptimizer<TGenotype, TFenotype>*)pc_optimizer;
}//CSteadyStateOptimizer<TGenotype, TFenotype> * OptimizerUtils::pcGetSteadyStateOptimizer(CProblem<TGenotype, TFenotype> *pcProblem, CLog *pcLog, uint32_t iRandomSeed, istream *psSettings, CError *pcError, bool bIsObligatory)
...
template CSteadyStateOptimizer<CBinaryCoding, CBinaryCoding> * OptimizerUtils::pcGetSteadyStateOptimizer(CProblem<CBinaryCoding, CBinaryCoding>*, CLog*, uint32_t, istream*, CError*, bool);
