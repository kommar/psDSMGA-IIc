...
CPopulationOptimizer<TGenotype, TFenotype>::CPopulationOptimizer(CProblem<TGenotype, TFenotype> *pcProblem, CLog *pcLog, uint32_t iRandomSeed)
	: CPopulationSizingSingleOptimizer<TGenotype, TFenotype>(pcProblem, pcLog, iRandomSeed)
{
...
CPopulationOptimizer<TGenotype, TFenotype>::CPopulationOptimizer(CPopulationOptimizer<TGenotype, TFenotype>* pcOther)
	: CPopulationSizingSingleOptimizer<TGenotype, TFenotype>(pcOther)
{
...
template <class TGenotype, class TFenotype>
double CPopulationOptimizer<TGenotype, TFenotype>::dComputeAverageFitnessValue()
{
	double d_sum = 0;
	...
	for (uint32_t i = 0; i < i_population_size; i++)
	{
		...
		d_sum += pc_individual->dGetFitnessValue();
	}//for (uint32_t i = 0; i < i_population_size; i++)
...
template <class TGenotype, class TFenotype>
void CPopulationOptimizer<TGenotype, TFenotype>::v_sort_population()
{
	sort
	(
		ppc_population + 0, 
		ppc_population + i_population_size, 
		[&](CIndividual<TGenotype, TFenotype> *pcIndividual0, CIndividual<TGenotype, TFenotype> *pcIndividual1)
		{
